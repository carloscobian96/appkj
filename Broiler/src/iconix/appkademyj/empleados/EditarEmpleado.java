...
public class EditarEmpleado extends JInternalFrame {
	...
	private final JRadioButton rdbtnPagaPorHora;
	private final JRadioButton rdbtnSalarioQuincenal;
	private final EmpleadoOld e;
	private final JTextField dedField;
	private final JTextField credField;
	private final JLabel lblDed;
	private final JLabel lblCred;
	private final JCheckBox regBox;

	public EditarEmpleado(final EmpleadoOld e, final DatabaseConnect d) {
		setClosable(true);
		this.e = e;
		...
		rdbtnSalarioQuincenal = new JRadioButton("Salario Quincenal");
		...
		rdbtnPagaPorHora = new JRadioButton("Paga por Hora");
		...
		lblDed = new JLabel("Ded:");
		lblDed.setFont(new Font("Dialog", Font.PLAIN, 20));
		final GridBagConstraints gbc_lblDed = new GridBagConstraints();
		gbc_lblDed.anchor = GridBagConstraints.WEST;
		gbc_lblDed.insets = new Insets(0, 0, 5, 5);
		gbc_lblDed.gridx = 5;
		gbc_lblDed.gridy = 7;
		panel.add(lblDed, gbc_lblDed);
		...
		final JButton btnCrear = new JButton("Actualizar");
		btnCrear.addActionListener(new ActionListener() {
			...
			public void actionPerformed(final ActionEvent arg0) {
				try {
					...
					if (verify()) {
						final EmpleadoOld em = new EmpleadoOld(e.getId(), nombreField.getText(),
								Integer.parseInt(ssnField.getText()), Integer.parseInt(dependientesField.getText()),
								nacimientoField.getText(), empleoField.getText(), telefonoField.getText(),
								fisicaField.getText(), postalField.getText(), posicionField.getText(),
								estadoCivilField.getText(), acctField.getText(), rdbtnSalarioQuincenal.isSelected(),
								salario, rate, Double.parseDouble(taxField.getText()), regBox.isSelected(), fica,
						...
						em.update(d);
						JOptionPane.showMessageDialog(null, "Empleado Actualuzado Exitosamente!", "Exito",
								JOptionPane.INFORMATION_MESSAGE);
						dispose();
					}
				} catch (NumberFormatException | HeadlessException | SQLException e) {
					e.printStackTrace();
					ErrorFrame frame = new ErrorFrame(e, "Error Editando Empleado");
		...
		btnCancelar.setIcon(new ImageIcon(
				EditarEmpleado.class.getResource("/org/jdesktop/swingx/plaf/basic/resources/error16.png")));
	...
	public boolean verify() {
		if (nombreField.getText().equals("")) {
			...
			return false;
		} else if (acctField.getText().equals("") || !isNumeric(taxField.getText())) {
			JOptionPane.showMessageDialog(null, "Campo Vacio", "Incompleto", JOptionPane.WARNING_MESSAGE);
			acctField.requestFocus();
			return false;
		} else if (salarioField.getText().equals("")) {
			JOptionPane.showMessageDialog(null, "Campo Vacio", "Incompleto", JOptionPane.WARNING_MESSAGE);
	...
	public void populate() {
		...
		ssnField.setText(e.getSSN().toString());
		dependientesField.setText(e.getNumDependentes().toString());
		nacimientoField.setText(e.getFechaNacimiento());
		empleoField.setText(e.getFechaEmpleo());
		telefonoField.setText(e.getTelefono());
		posicionField.setText(e.getPosicionTrabajo());
		estadoCivilField.setText(e.getEstadoCivil());
		acctField.setText(e.getChartOfAcct());
		postalField.setText(e.getDireccionPostal());
		fisicaField.setText(e.getDireccionFisica());
		planCancerField.setText(e.getCancer().toString());
		planMedicoField.setText(e.getPlanMedico().toString());
		taxField.setText(e.getTax().toString());
		totalPagadoField.setText(e.getTotalPagado().toString());
		dedField.setText(e.getDed().toString());
		credField.setText(e.getCred().toString());
		regBox.setSelected(e.getReg());

		if (e.isSalarioFijo()) {
			rdbtnSalarioQuincenal.setSelected(true);
			salarioField.setText(e.getSalarios().toString());
		} else {
			rdbtnPagaPorHora.setSelected(true);
			salarioField.setText(e.getHourRate().toString());
		}
